...
console.log(lowCaseAnimalNames.map(item => item.toLowerCase()));
...
lowPopulationAnimals = zooAnimals.filter(word => word.population < 5);
...
const combine = (accumulator, currentValue) => accumulator + currentValue;
var population = []
zooAnimals.forEach(function(item){
  population.push(item.population)
})
populationTotal = population.reduce(combine)
